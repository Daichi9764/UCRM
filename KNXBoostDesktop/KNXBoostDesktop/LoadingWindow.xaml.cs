...
namespace KNXBoostDesktop
{
    public partial class LoadingWindow
    {
        ...
        {
                UpdateTaskName("Tâche 1/3");
                await App.Fm.FindZeroXml(this).ConfigureAwait(false);

                UpdateTaskName("Tâche 2/3");
                await MyNameCorrector.CorrectName(this).ConfigureAwait(false);

                UpdateTaskName("Tâche 3/3");
                await ExportUpdatedNameAddresses.Export(this).ConfigureAwait(false);

                Dispatcher.Invoke(() =>
                {
                    UpdateTaskName("Chargement terminé !");
